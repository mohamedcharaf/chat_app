...
class FireData1 {
  ...
  Future<void> createRoom(String email) async {
    ...
    await FirebaseFirestore.instance
        ...
        .doc(msgId)
        .set(message.toJson());
         firestore.collection('rooms').doc(roomId).update({'lastMessage':msg,'lastMessageTime':DateTime.now().millisecondsSinceEpoch.toString()});
  } catch (e) {
   ...
   Future  sendGMsg( String msg,String groupId,{String? type}) async{
    ...
    await FirebaseFirestore.instance
        ...
        .doc(msgId)
        .set(message.toJson());
         firestore.collection('groups').doc(groupId).update({'lastMessage':msg,'lastMessageTime':DateTime.now().millisecondsSinceEpoch.toString()});
  } catch (e) {
    print('Erreur lors de lenvoi du message : $e');
  }
...
Future readMessage(String roomId , String msgId) async {
  ...
  collection('messages').doc(msgId).update({'read':DateTime.now().millisecondsSinceEpoch.toString()});
}

deletMsg(String roomId , List<String >msgs) async {
  if(msgs.length ==1 ){
    await firestore.collection('rooms').doc(roomId).collection('messages').doc(msgs.first).delete();
...
copyMsg (String roomId , List<String >msgs) async {
  for(var element in msgs){
    await firestore.collection('rooms').doc(roomId).collection('messages').doc(element);
